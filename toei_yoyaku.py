...
#検索対象日
delta = 35 #何日先までを対象にするか
youbi = (5,6,7) #(0：月曜～6：日曜、7：祝日)

#対象urlとログイン情報
url = "https://yoyaku.sports.metro.tokyo.jp/user/view/user/homeIndex.html"
USER = 86299810
PASS = 75315987
#通知先
...
try:
    ...
    #予約「種目から探す」をクリック
    browser.find_element_by_id("purposeSearch").click()
    browser.implicitly_wait(5)
    ...
    for id_checked in id_checkedes:
        ...
        if att_name == "layoutChildBody:childForm:purposeSearchItems:0:selectItemsItems:0:selectItems:2:checked":#ハード
            id_checked.click()
        elif att_name == "layoutChildBody:childForm:purposeSearchItems:0:selectItemsItems:0:selectItems:3:checked":#人工芝
            id_checked.click()
    ...
    for t_day in t_days:
        ...
        while len(id_goNextPagers) >= 1:
            ...
            for tag_table in tag_tables:#施設ごとに空きを確認
                ...
                id_tzoneStimeLabels = tag_table.find_elements_by_id("tzoneStimeLabel")#時間が書いてある要素リストを取得
    ...
    if len(list_aki) >= 1:
        ...
        data = "空きはありません。"
    print(data)
    slack.post("都営空き", data, slack_post_url)
    #line.post("都営空き", data, line_token)
